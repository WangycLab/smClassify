...
#   1) Parse a genome GTF file and extract gene_id / symbol / KO mapping.
#   2) Build gene alias table and KO-to-gene universe for enrichment.
#   3) Download / cache KEGG pathway & module term–KO mappings.
#   4) Read pre-computed marker genes per cluster and map to gene IDs.
#   5) Run KEGG ORA (pathway & module) for each Seurat cluster.
#   6) Write per-cluster results and combined result tables.
#   7) Annotate with KEGG pathway hierarchy and generate a
#      category-faceted overview dot plot.
#
# Notes:
#   - All thresholds and parameters (e.g., PADJ_CUT, MIN_COUNT,
#     TOP_N_PLOT, filtering rules) are identical to the original script.
#   - Only file paths have been generalized for GitHub use.
# ============================================================
...
# You can override OUTDIR / GTFFILE by defining them BEFORE sourcing
# this script. If not defined, the following defaults are used:
OUTDIR  <- get0("OUTDIR",  ifnotfound = file.path(getwd(), "ora_out"))
GTFFILE <- get0("GTFFILE", ifnotfound = file.path(getwd(), "M1_selected_genome_fixed.gtf"))

# ----------------------------------------------------------------
# Global parameters (unchanged)
# ----------------------------------------------------------------
PADJ_CUT   <- 0.25   # p.adjust cutoff for enrichment filtering
MIN_COUNT  <- 1      # minimum gene count per term
TOP_N_PLOT <- 20     # top N terms shown in per-cluster dotplots
...
invisible(lapply(
  list(OUTDIR, DIR_CACHE, DIR_ENR, DIR_LOG),
  dir.create,
  recursive = TRUE,
  showWarnings = FALSE
))
...
# .norm_id:
#   Normalize an ID by uppercasing and replacing "_" with "-".
.norm_id <- function(x) toupper(gsub("_","-", as.character(x), fixed = TRUE))

# .blankish:
#   TRUE for NA / empty / whitespace-only strings.
.blankish <- function(x){
  x <- as.character(x)
  x[is.na(x)] <- ""
  !nzchar(trimws(x))
...
# get_attr_gtf:
#   Extract a given attribute (e.g., locus_tag, Name, gene, product)
#   from the GTF attributes column using a regular expression.
...
# split_kos:
#   Split KO strings into individual KEGG KO IDs (Kxxxxx).
...
# .safe_dir:
#   Sanitize strings to be used safely as directory names.
.safe_dir <- function(x){
  gsub('[\\\\/:*?"<>|]', "_", as.character(x))
}

# ============================================================
# Step A. Parse GTF and build gene–KO mapping
# ============================================================
message("Step A: Parsing GTF and building gene–KO mapping ...")
...
keg <- ifelse(!.blankish(keg1), keg1,
              ifelse(!.blankish(keg2), keg2, keg3))

# Build gene_id and symbol
gene_id <- .norm_id(lt)
...
# Gene alias table: alias -> gene_id
...
# ============================================================
# Step B. Load reference and KEGG term mappings
# ============================================================
message("Step B: Loading reference and KEGG mappings ...")
...
# Try to load cached KEGG mapping if available
kegg_cache_ok <- all(file.exists(file.path(
  DIR_CACHE, c("PATH2KO.rds","MOD2KO.rds","PATH_NM.rds","MOD_NM.rds")
)))
...
if (kegg_cache_ok) {
  ...
  # Fetch and convert KEGG pathway/module to KO mappings
...
message(" KEGG reference saved / loaded from cache.")

# ============================================================
# Step C. Read markers and prepare gene sets
# ============================================================

# read_markers:
#   Normalize gene column, map via alias_long to gene_id,
#   and return (gene_id, cluster).
...
# Read top marker genes per cluster (pre-computed)
...
# get_interest_universe:
#   For a subset of markers, compute KO "interest" set and KO "universe".
...
# run_enrich:
#   Run clusterProfiler::enricher on a set of KO terms.
run_enrich <- function(interest, universe, TERM2GENE, TERM2NAME,
                       padj_cut = 0.25, minCount = 1){
...
# plot_dot:
#   Simple per-cluster dotplot of enriched terms.
...
# ============================================================
# Step D. Run enrichment for each cluster
# ============================================================
message("Step D: Running KEGG ORA per cluster ...")
...
for (cl in clusters) {
  ...
  res_p <- run_enrich(KU$interest, KU$universe, PATH2KO, PATH_NM,
                      PADJ_CUT, MIN_COUNT)
  fwrite(res_p$all,
         file.path(cl_dir, "KEGG_pathway_results_all.tsv"),
         sep = "\t", quote = FALSE, na = "")
  fwrite(res_p$keep,
         file.path(cl_dir, "KEGG_pathway_results_filtered.tsv"),
         sep = "\t", quote = FALSE, na = "")
  ...
  res_m <- run_enrich(KU$interest, KU$universe, MOD2KO, MOD_NM,
                      PADJ_CUT, MIN_COUNT)
  fwrite(res_m$all,
         file.path(cl_dir, "KEGG_module_results_all.tsv"),
         sep = "\t", quote = FALSE, na = "")
  fwrite(res_m$keep,
         file.path(cl_dir, "KEGG_module_results_filtered.tsv"),
         sep = "\t", quote = FALSE, na = "")
...
fwrite(PA_all,
       file.path(comb_dir, "KEGG_pathway_results_all_clusters_all.tsv"),
       sep = "\t", quote = FALSE, na = "")
fwrite(PA_keep,
       file.path(comb_dir, "KEGG_pathway_results_all_clusters_filtered.tsv"),
       sep = "\t", quote = FALSE, na = "")
fwrite(MO_all,
       file.path(comb_dir, "KEGG_module_results_all_clusters_all.tsv"),
       sep = "\t", quote = FALSE, na = "")
fwrite(MO_keep,
       file.path(comb_dir, "KEGG_module_results_all_clusters_filtered.tsv"),
       sep = "\t", quote = FALSE, na = "")

utils::capture.output(
  sessionInfo(),
  file = file.path(DIR_LOG, "sessionInfo.txt")
)

# ============================================================
# Step E. High-level filtering and faceted overview plot
# ============================================================

library(tidyverse)
df <- readr::read_tsv(
  file.path(DIR_ENR, "combined", "KEGG_pathway_results_all_clusters_all.tsv"),
  show_col_types = FALSE
)

# KEGG pathway hierarchy file (pre-curated)
hier <- read.csv("kegg_pathway_hierarchy_clean_v4.csv")

df_anno <- df %>%
  mutate(
    ID = sub("^ko", "map", ID),
    Count = if (!"Count" %in% names(.))
      as.numeric(sub("/.*$", "", GeneRatio)) else Count,
    GeneRatio_num = {
...
# ------------------------------------------------------------
# Remove human-disease-like and overly broad pathways
# ------------------------------------------------------------
...
# Broad KEGG pathway IDs to be removed
big_pathway_ids <- c(
  "ko01100","map01100",
  "ko01110","map01110",
  "ko01120","map01120",
...
# ------------------------------------------------------------
# Top per-cluster pathways for plotting
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Faceted overview dot plot
# ------------------------------------------------------------

library(ggh4x)

strip_colors <- c("#f0f0f0", "#d9f0d3", "#c6dbef", "#fdd0a2")
...
ggsave("DEGs_cluster_KEGGpathways_overview.png",
       plot = p, width = 10.5, height = 11, dpi = 500)
ggsave("DEGs_cluster_KEGGpathways_overview.pdf",
       plot = p, width = 10.5, height = 11)
