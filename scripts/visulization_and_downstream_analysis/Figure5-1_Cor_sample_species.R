...
## Correlation results for Muribaculum gordoncarteri 
cor_res <- corr_matrix["Muribaculum gordoncarteri", ]
...
# Save results
write.csv(data.frame(Species = names(cor_res), Correlation = cor_res),
          "Muribaculum_gordoncarteri_correlation_relative.csv", row.names = FALSE)
...
pheatmap::pheatmap(corr_matrix,
         ...
         fontsize = 6,
         color = colorRampPalette(c("blue","white","red"))(50),
         filename = "Correlation Heatmap (Species Relative Abundance).png", width = 8, height = 6)

library(ComplexHeatmap)
dir.create("Figure5", showWarnings = FALSE, recursive = TRUE)

png("Correlation_Heatmap_Species_Relative_Abundance.png",
...
if(length(sel_species) > 0){
  ...
  pheatmap(cor(mat_sel, method="spearman"),
...
pheatmap::pheatmap(corr_top20,
         ...
         fontsize = 8,
         color = colorRampPalette(c("blue","white","red"))(50),
         filename = "Correlation_Top20_Species.pdf", width = 8, height = 7)
...
pheatmap::pheatmap(corr_top20,
         ...
         fontsize = 8,
         color = colorRampPalette(c("blue","white","red"))(50),
         filename = "Correlation_Top20_Species.png", width = 2400/300, height = 2400/300)


library(ComplexHeatmap)

# Row order (species order)
...
suppressPackageStartupMessages({
  ...
  library(purrr); library(pheatmap); library(patchwork); library(gridExtra)
})

## 1) Define gene sets (original order preserved)
## Define gene sets (names use hyphens, gene order preserved exactly)

genesets <- list(
  # Short-chain fatty acid (SCFA) metabolism
  Butyrate = c("hbd","crt","bcd","etfB","buk","atoD"),
  Acetate  = c("pta","ackA","acs"),
  Propionate = c("pccB","epi","mutA","mutB","prpC","prpB","acnA","icd","acsA","ldhA"),
  ...
  "Bile-Total"          = c("cbh","bshA","tolC","acrA","acrB","ompR","emrA","norM","mdtK","cusB","mexB"),
  
  # Protein/amino acid metabolism
  Stickland = c("grdA","grdB","grdC","grdD","fldA","fldB"),
  "Nitrogen-Metabolism" = c("gdhA","gltB","gltD","glnA","glnB","glnD","glnK",
  ...
  "LCFA-Auxiliary" = c("fadD3","acsA","acsC","acsD","acsE","accA","accB","accC","accD","fadM"),
  "LCFA-Beta-Oxidation" = c("hbd","crt","mutB","epi","etfB","fadN"),
  "LCFA-Transport-Efflux" = c("acrB","bepC","bepF","bepG","tolC","mexB","mdtA","mdtB","mdtC"),
  "LCFA-Regulation" = c("fadR","fnr","crp","arcA","arcB"),
  "LCFA-Other-Aux" = c("caiA","caiT","acdA","pflB","fixA","fixB","fixX"),
  
  # Stress response / resistance
  Stress = c("sodA","sodB","sodC","katA","katE","rbr","dnaK","groL","groS","clpB","clpP","grpE"),
  "Efflux-Resistance" = c("tolC","acrA","acrB","ompR","emrA","norM","mdtK","cusB","mexB"),
  
  # Propionate sub-pathways (two lists kept; renamed to avoid duplicate name)
  "Propionate-MCM" = c("pccB","epi","mutA","mutB"),
  "Propionate-MCC" = c("prpC","prpB","acnA","icd","emrA","norM","mdtK","cusB","mexB"),
  "Propionate-Acrylate" = c("acsA","ldhA")
)



# Get gene names actually present in object
...
sel_seu@active.ident <- as.factor(sel_seu$species)

# Assume you already obtained the order from heatmap
# ordered_species <- rownames(corr_top20)[row_order]
...
DotPlot(sel_seu, features = c(modules <- c(
  modules <- c(
    "Acetate2",
    "Propionate-MCM19",
    "Propionate-MCC20",
  ...
  labs( title="Key Metabolic and Stress Pathways in Top 20 Species")

p
ggsave( "Key Metabolic and Stress Pathways_top20 species.png", p, w=10, h=7.5, dpi = 600)

ggsave( "Key Metabolic and Stress Pathways_top20 species.pdf", p, w=10, h=7.5)
