# ======================================================
# Amino Acid Metabolism x Per-metabolite boxplots (WT vs DB)
...
#  targets (standardize & remove duplicates)
targets_raw <- c(
  # Urea cycle core
...
#1) Read files & convert to long format
...
# 3) Select target metabolites only (case-insensitive; standardized names)
...
for (m in all_mets) {
  ...
  # Compare WT vs DB for each tissue only (only if both groups exist)
