...
#
# This script starts from the Figure 3 functional-cluster annotated
# Seurat object and generates:
#   1) Sankey plot: intestinal region/phenotype group -> functional cluster
#   2) Functional-cluster composition by phenotype
#   3) Functional-cluster composition by intestinal region
#   4) Total cell number per functional cluster
#
# ============================================================
...
#   Cecum-WT-1, Cecum-WT-2, Cecum-DB-1, Cecum-DB-2, etc.
...
# Append any unexpected labels rather than dropping them.
...
# Rebuild metadata after all metadata updates. This avoids stale celltype values.
...
# Keep phenotype and region colors consistent across Figure 3 panels.
pal_pheno <- c(
  WT = "#3498DB",
  DB = "#D94F45"
)
...
# Functional-cluster colors extracted from the UMAP color scheme.
...
# Extract cluster ID from functional-cluster label.
celltype_cluster_id <- sub("^([0-9]+).*", "\\1", celltype_levels)
names(celltype_cluster_id) <- celltype_levels

if (any(!celltype_cluster_id %in% names(cluster_cols_nature))) {
  stop(
    "Some functional-cluster labels do not start with cluster IDs 0-7:\n",
    paste(celltype_levels[!celltype_cluster_id %in% names(cluster_cols_nature)], collapse = "\n")
  )
}
...
# ============================================================
# The original logic is retained: Sankey values are proportions calculated
# by functional cluster, i.e. prop.table(table(Group, Celltype), margin = 2).
...
p_sankey
# Export PNG if webshot2/Chrome is available. The HTML is always saved.
tryCatch(
  {
    webshot2::webshot(
      url = html_file,
  ...
  error = function(e) {
    message("[Figure 3d] Sankey HTML saved, but PNG export failed: ", conditionMessage(e))
  }
)
