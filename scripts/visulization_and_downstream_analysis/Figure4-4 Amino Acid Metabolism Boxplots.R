# Figure 4E — Amino Acid Metabolism Boxplots (WT vs DB)
# - Input: Metabolomics_Serum.xlsx, Metabolomics_CecalContent.xlsx
# - Column format example:
#     Colon-DB1, Colon-WT1, Cecum-DB2, Rectum-WT3, Serum-WT4, Serum-DB6
# - Tissue = prefix before "-", Treatment = WT/DB extracted after "-"
# - Welch’s t-test (two-sided), FDR per tissue
# - Per-metabolite boxplots for Serum / Gut
# - Saves source data + stats + ylim per metabolite
# ======================================================================
...
# ======================================================================
# 1. Define target metabolites (standardized + deduplicated)
# ======================================================================
targets_raw <- c(
  "Glutamic acid","Glutamine","Ornithine","Citrulline",
  "N-acetyl-L-glutamate","D-glutamic acid","DL-arginine","Homo-L-arginine",
  "Phenylpyruvic acid","4-Hydroxyphenyl-2-propionic acid","Hydroxyphenyllactic acid",
  "Propionylcarnitine",
  "N-Phenylacetylglutamic acid","N-Phenylacetylaspartic acid",
  "Isovalerylcarnitine","Ketoleucine"
)

# Deduplicate (case-insensitive)
targets_lc <- tolower(targets_raw)
targets <- targets_raw[match(unique(tolower(targets_raw)), tolower(targets_raw))]

fname_prefix <- "Amino Acid Metabolism"
...
# Replace any mis-labeled "Seurm" to "Serum"
...
# ======================================================================
# 3. Parse Tissue / Treatment from new column naming format
#    Example: "Colon-DB3" → Tissue="Colon", Treatment="DB"
# ======================================================================
dat_long <- all_long %>%
  rename(Metabolite = MS2.name) %>%
  mutate(
    Metabolite = ifelse(is.na(Metabolite) | !nzchar(Metabolite), MS1.name, Metabolite),
    Tissue     = str_extract(Sample, "^[A-Za-z]+"),
    Treatment  = case_when(
...
# ======================================================================
# 4. Select target metabolites (case-insensitive)
# ======================================================================
...
# ======================================================================
# 5. Welch’s t-test per tissue × metabolite
# ======================================================================
...
# ======================================================================
# 6. Unified group labels (e.g., "Colon-WT", "Serum-DB")
# ======================================================================
...
# ======================================================================
# 7. Compute y-axis padding (avoid clipping significance labels)
# ======================================================================
ypad_ratio <- 0.20
min_span_abs <- 1e-6
...
pad_limits <- function(y) {
  ...
  if (!is.finite(span) || span <= 0) {
    return(c(ymin - 0.5, ymax + 0.5*(1+ypad_ratio)))
  }
  span_use <- max(span, min_span_abs)
  return(c(ymin, ymax + span_use * ypad_ratio))
}

serum_ylim <- source_serum %>%
  group_by(Metabolite) %>%
  summarise(ymin=min(Intensity), ymax=max(Intensity), .groups="drop") %>%
  rowwise() %>%
...
# ======================================================================
# 9. Plot Serum metabolites (each metabolite → one PDF + PNG)
# ======================================================================
for (m in all_mets) {
...
# ======================================================================
# 10. Plot Gut metabolites (Cecum / Colon / Rectum)
# ======================================================================
for (m in all_mets) {
  ...
  # Compare WT vs DB in each tissue if both exist
  cmp_list <- list()
