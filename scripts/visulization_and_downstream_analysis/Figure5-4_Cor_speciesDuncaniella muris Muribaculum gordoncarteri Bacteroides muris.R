...
cor_species<-c( 
               "Duncaniella muris"   ,   "Muribaculum gordoncarteri"   , 
               "Bacteroides muris" )
...
p<-DimPlot(cor_seu, reduction = "umap", label = F, pt.size = 0.1,
        group.by = "species") +
...
comp_tab <- df %>%
  ...
  group_by(cluster) %>%
  mutate(freq = count / sum(count))   # ???Ա???

# Plot (proportional stacked bar chart) 
ggplot(comp_tab, aes(x = cluster, y = freq, fill = species)) +
  geom_bar(stat = "identity", position = "fill") +  # Use ??fill?? to ensure proportions sum to 1
...
cell_type_cols <- c(
  ...
  "#FF1493", "#FF4500", "#32CD32", "#3E8E41")
p_Subtype <- DimPlot(cor_seu, reduction = "umap", label = FALSE, pt.size = 0.1, 
...
p<-DotPlot(cor_seu, features = c(modules <- c(
  modules <- c(
...
p
ggsave("Cor_Dot plot of species-specific functional gene module activity.png", p, 
       width = 9, height = 3.5, dpi = 300)


ggsave("Cor_Dot plot of species-specific functional gene module activity.pdf", p, 
       width = 9, height = 3.5)
...
p<-DotPlot(cor_seu, features = all_genes,
group.by = "cellsubtype",
scale = T) + 
  RotatedAxis()+
  scale_size_continuous(range = c(2, 8))+
...
p
ggsave("Cor_Dot plot of species-specific functional gene module activit_genes.png", p, 
       width = 13, height = 3.5, dpi = 300)
