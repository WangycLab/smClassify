...
read_markers <- function(DT, alias_long) {
  ...
  gene_input[is.na(gene_input)] <- DT$gene_id[is.na(gene_input)]
  gene_input[is.na(gene_input)] <- DT$symbol[is.na(gene_input)]

  DT[, gene_input := gene_input]
...
get_interest_universe <- function(sub_df, KO_by_gene, gene_universe) {
  ...
  universe <- if (length(universe) < 10) unique(KO_by_gene$ko) else universe
