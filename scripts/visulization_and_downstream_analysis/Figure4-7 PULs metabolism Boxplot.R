# ======================================================
# PULs metabolism - Per-metabolite boxplots (WT vs DB)
...
targets_raw <- c(
  "Trehalose","Trehalose 6-phosphate","D-Maltose","Raffinose"
)
...
#   filenames updated to your current naming scheme
...
##    compatible with headers like:
##    Colon-DB1, Cecum-WT3, Rectum-DB5, Serum-WT2, Serum-DB4 ...
dat_long <- all_long %>%
...
## 7) Plotting – Serum: single plot per metabolite 
for (m in all_mets) {
  ...
  p_serum <- ggplot(df_serum, aes(x = Group, y = Intensity, fill = Treatment)) +
    geom_boxplot(outlier.shape = NA, width = 0.6,
                 color = "black", alpha = 0.7, size = 0.6) +
    geom_jitter(aes(color = Treatment),
                width = 0.12, size = 1.6, alpha = 0.6) +
  ...
  ggsave(file.path(plot_dir, paste0(fn, ".pdf")), p_serum,
         width = 5.4, height = 6, dpi = 300)
  ggsave(file.path(plot_dir, paste0(fn, ".png")), p_serum,
         width = 5.4, height = 6, dpi = 300)
}

## 7b) Plotting – Gut: single plot per metabolite
for (m in all_mets) {
  ...
  ggsave(file.path(plot_dir, paste0(fn, ".pdf")), p_gut,
         width = width_n, height = 6, dpi = 300)
  ggsave(file.path(plot_dir, paste0(fn, ".png")), p_gut,
         width = width_n, height = 6, dpi = 300)
}
...
# （逻辑保持原样，尽管对 PULs 实际上会是空结果）

# Vectorized division
v_div <- function(num, den) {
  num <- as.numeric(num); den <- as.numeric(den)
  out <- num / den
  bad <- !is.finite(out) | !is.finite(num) | !is.finite(den) |
...
wide_target <- dat_target %>%
  mutate(
    # Standardize DPA naming: Docosapentaenoic acid (22n-3) -> Docosapentaenoic acid
...
# 8.2 Ensure required columns exist for calculation (add NA columns if missing)
...
ratio_stats <- ratio_long %>%
  ...
  summarise(
    ...
    p_val  = safe_t_p(
      Value[Group %in% paste0(unique(Tissue), "-WT")],
...
write.csv(
  ratio_stats,
  file.path(plot_dir, paste0(fname_prefix, "_RATIOS_WT_vs_DB_stats_ttest_all.csv")),
  row.names = FALSE
)

write.csv(
  ratio_long,
  file.path(plot_dir, paste0(fname_prefix, "_RATIOS_SourceData_points.csv")),
  row.names = FALSE
)

# RATIO-specific color scheme
col_fill_ratio  <- c("WT" = "#1A1A1A", "DB" = "#E69F00")
col_point_ratio <- c("WT" = "#1A1A1A", "DB" = "#E69F00")
shape_ratio     <- c("WT" = 16,       "DB" = 17)

# 8.6 y-axis limits for ratios
...
write.csv(
  ratio_ylim,
  file.path(plot_dir, paste0(fname_prefix, "_RATIOS_ylim_by_ratio.csv")),
  row.names = FALSE
)

# 8.7 Plotting PULs ratios - Serum
...
for (r in all_ratios) {
  ...
  df_s <- df_s %>%
    mutate(
      ...
      Treatment = factor(
        ifelse(str_detect(Group, "WT$"), "WT", "DB"),
        levels = c("WT","DB")
      )
    )
  ...
  ggsave(file.path(plot_dir, paste0(fn, ".pdf")), p_s,
         width = 5.4, height = 6, dpi = 300)
  ggsave(file.path(plot_dir, paste0(fn, ".png")), p_s,
         width = 5.4, height = 6, dpi = 300)
}

# 8.8 Plotting PULs ratios - Gut
for (r in all_ratios) {
  df_g <- ratio_long %>%
    filter(Tissue %in% c("Cecum","Colon","Rectum"), Ratio == r)
  if (nrow(df_g) == 0) next
  ...
  p_g <- ggplot(df_g, aes(x = Group, y = Value, fill = Treatment)) +
    geom_boxplot(outlier.shape = NA, width = 0.6,
                 color = "black", alpha = 0.7, size = 0.6) +
    geom_jitter(aes(color = Treatment, shape = Treatment),
                width = 0.15, size = 1.6, alpha = 0.75) +
    scale_fill_manual(values = col_fill_ratio) +
    scale_color_manual(values = col_point_ratio) +
    scale_shape_manual(values = shape_ratio) +
    coord_cartesian(ylim = y_limits) +
  ...
  ggsave(file.path(plot_dir, paste0(fn, ".pdf")), p_g,
         width = width_n, height = 6, dpi = 300)
  ggsave(file.path(plot_dir, paste0(fn, ".png")), p_g,
         width = width_n, height = 6, dpi = 300)
}
